...
* **数据量超限**：当查询数据范围过大时，接口可能返回精简数据，说明文件中会出现提示：`"检测到您的数据范围较大，由于系统限制，现为您返回的是精简后的部分数据"`

> **示例：** “查询 A、B、C、D、E、F 六只股票的营收” -> 仅返回前五只。

### 4. 调用配额

* **每日调用限额**：约 50 次/日（以东方财富妙想平台实际策略为准）
* **超限表现**：接口返回 `rate limit`、`quota exceeded` 或 `调用次数已达上限`
* **建议**：高频场景配置多 Token 轮换（`EM_API_KEY_POOL`），外层封装实现降级策略
